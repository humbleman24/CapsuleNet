...
def Mnist_Train():
    ...
    model_Cfg = Mnist_Capsnet_Cfg()
    model = CapsNet(model_Cfg).to(DEVICE)
    ...
    for epoch in range(train_Cfg.num_epochs):
        ...
        if (
            ...
            torch.save(
                checkpoint,
                Path(train_Cfg.output_dir, f"Mnist_epoch{epoch}.pth").mkdir(
...
def test(
    ...
    tqdm.write(
        "Epoch: [{}/{}], test accuracy: {:.6f}, test loss: {:.6f}".format(
            epoch + 1,
            num_epochs,
            accuracy,
